...
class OpenLocaleCommand(sublime_plugin.TextCommand):
	def run(self, edit=None, url=None):
		selection = OpenUrlCommand(self.view).selection()
		config    = sublime.load_settings("open_url.sublime-settings")
		path_to   = selection.split('/')
		file      = path_to.pop().split('.')[0]

		path_to.append(file)
		path_to = config.get('locales_path') + '/'.join(path_to) + '.ru_RU.yaml'

		self.view.run_command('open_url', {
			"url": path_to
		});
